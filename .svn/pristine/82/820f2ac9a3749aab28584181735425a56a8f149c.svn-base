...
{
    public partial class ConferenceAvaialbleRooms : Form
    {
        string tempconf;

        NS_ConfAdmin.StrongTypesNS.DS_CCODEDataSet ds_ccode;
      
        public ConferenceAvaialbleRooms()
        {
            InitializeComponent();
        }

        private void ConferenceAvaialbleRooms_Load(object sender, EventArgs e)
        {
        ...
        private void btn_print_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    ds_rooms = Proxy.ConferenceAdmin.conference_available_rooms(txt_conf_code.Text.ToString(),temphall,tempres, cbuilding,start_dte, end_dte, double_rooms);

                    if (ds_rooms.tt_conf_rooms.Count > 0)
                    {
                        DataView dvReport = new DataView(ds_rooms.tt_conf_rooms);
                        string temptitle = txt_conf_descrip.Text.ToString() + " available rooms from  " + start_dte.ToShortDateString() + " until  " + end_dte.ToShortDateString();

                        Reports frm_report = new Reports(false, "ConferenceRooms", dvReport, temptitle);
                        frm_report.Show();
