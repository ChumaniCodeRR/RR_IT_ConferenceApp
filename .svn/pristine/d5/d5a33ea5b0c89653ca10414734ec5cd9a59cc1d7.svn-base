    ...
    public partial class Planner : Form
    {
        ...
        void dgvBookings_SelectionChanged(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (dgvBookings.SelectedCells.Count > 0)
                {
                    ...
                    if (index == cFloor.Index || index == cRoomNo.Index || index == cBeds.Index || index == cNotes.Index || index == cNAFBEDS.Index)
                    {
                        dgvBookings.ClearSelection();
                    }
            ...
            try
            {
                if (e.RowIndex != -1)
                {
                    if (e.ColumnIndex != cFloor.Index && e.ColumnIndex != cRoomNo.Index &&
                        e.ColumnIndex != cBeds.Index && e.ColumnIndex != cNotes.Index && e.ColumnIndex != cNAFBEDS.Index)
                    {
                        //Check if room is reserved
                            ...
                            !dgvBookings[e.ColumnIndex, e.RowIndex].Value.ToString().Equals(string.Empty))
                        {
                            ...
                            Get_Reservation(folio, dgvBookings[e.ColumnIndex, e.RowIndex].Value.ToString());
        ...
        void SetHorizontalHeaders()
        {
            ...
            for (int i = 0; i < dgvBookings.Columns.Count; i++)
            {
                ...
                if (c.Name != cFloor.Name && c.Name != cRoomNo.Name && c.Name != cBeds.Name && c.Name != cNotes.Name && c.Name != cNAFBEDS.Name)
                {
        ...
        void RefreshBookings()
        {
            ...
            foreach (ds_booking_summaryDataSet.tt_booking_summaryRow booking in Global.Global.ds_bookings.tt_booking_summary)
            {
                ...
                {
                    if (cell.ColumnIndex != 0 && cell.ColumnIndex != 1 && cell.ColumnIndex != 2 && cell.ColumnIndex != 3 && cell.ColumnIndex != 4)
                    {
                        ...
                        if (cellDate.Date >= booking.start_dte.Date && cellDate.Date <= booking.end_dte.Date.AddDays(-1)) //Exclude the date guests leave, hence endDate - 1
                        {
                            ...
                            cell.Tag = cellTag;
                            if (booking.folio == "-1") cell.Style.BackColor = Color.LightGreen;
                        }
                    }
                }
            }
        ...
        private void Get_Reservation(string folio,string descrip)
         {

            if (folio == "-1")
            {
                string confcode = descrip.Substring(0, descrip.IndexOf(":")).Trim();
                
                ConferenceSys.ConferenceAdmin.Conference_Updates.BlockBooking frmBlock = new ConferenceSys.ConferenceAdmin.Conference_Updates.BlockBooking(confcode, cbBuilding.SelectedValue.ToString(), cbHouse.SelectedValue.ToString(), cbHall.SelectedValue.ToString(), descrip.Trim());
                frmBlock.ShowDialog();
            }
            else
            {
                ...
                ConferenceSys.ConferenceAdmin.UpdateReserv frmReservation = new ConferenceSys.ConferenceAdmin.UpdateReserv(false, Global.Global.ds_single_booking.tt_reservation[0].ccode, Global.Global.ds_single_booking.tt_reservation[0].parentfolio, Global.Global.ds_single_booking.tt_reservation[0].folio, Utilities.AccessControl.HasUpdatePayments);
        ...
        private void txt_folio_KeyDown(object sender, KeyEventArgs e)
        {
            if (txt_folio.Text.Length > 0)
            {
                ...
                {
                    int folio;
                    if (int.TryParse(txt_folio.Text.ToString(), out folio))
                    {
                        Get_Reservation(txt_folio.Text, "");
                    }
                    else MessageBox.Show("Folio must contain numbers only", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void lblResidencePlannng_Click(object sender, EventArgs e)
        {
