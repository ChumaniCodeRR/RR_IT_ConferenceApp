    ...
    public partial class GroupReservations : Form
    {
        ...
        private void refresh_data()
        {
            try
            {
                ...
                for (int x = 0; x < ds_reservations.tt_reservations.Rows.Count;x++ )
                {
                    if(ds_reservations.tt_reservations[x].guests > 0)
                    {
                        countGuest += ds_reservations.tt_reservations[x].guests;
                    }
                }
                txt_total_guests.Text = countGuest.ToString();
        ...
        */
                    ...
                    if (MessageBox.Show("Are you sure you want to cancel selected reservation(s)?", "Multiple Cancel", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        foreach (DataGridViewRow row in dg_conferences.Rows)
                        {
                            if (row.Cells[ckCopy.Name].Value != null && (bool)row.Cells[ckCopy.Name].Value)
                            {
                                ...
                                newrow["parentfolio"] = row.Cells[cn_parentfolio.Name].Value.ToString();
        ...
        {
                NS_ConfAdmin.StrongTypesNS.ds_reservationDataSet ds_report = Proxy.ConferenceAdmin.reservation_report(true, ParentFolio);
                if (ds_report.tt_reservations.Rows.Count > 0)
                {
                    ConferenceSys.Reports frmReport = new ConferenceSys.Reports("ReservationReport", ds_report);
                    frmReport.Show();
        ...
        private void btn_delMain_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Are you sure you want to cancel this group from this conference?","Cancel Group", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                string feedback = Proxy.ConferenceAdmin.cancel_reservations(false, ParentFolio, "");
